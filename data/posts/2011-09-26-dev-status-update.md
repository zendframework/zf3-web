---
layout: post
title: 2011-09-26 Dev status update
date: 2011-09-26 22:00
update: 2011-09-26 22:00
...
- [RFC on configuration](http://framework.zend.com/wiki/display/ZFDEV2/Comparison+of+config+styles)
...
- If a dependency has a sane default, we would not include an argument in the constructor, but _would_ allow setter injection (which could be automated by the DI container)
...
- The component would directly access optional arguments from the Options instance it composes
 ...
 Note the word "Options". In discussions, we decided we should call this functionality "Options" as it denotes that the values are optional, and also to prevent nomenclature conflicts with the already existing `Zend\Config` component. We will be producing a poll on the wiki to do a final vote _very soon_.
...
### Module Directory Structure
 ...
 As such, our decision was that we should have a _recommended_ structure that satisfies the various use cases we originally brainstormed. Among these:
...
- Separation of configuration, if provided, from code
 ...
 As such, the following structure was _generally_ agreed upon as a recommendation:
 ...
 Public assets such as images, CSS, and JS could either be symlinked into the public tree, copied, or managed by an asset manager such as [Assetic](https://github.com/kriswallsmith/assetic).

 Regarding the various `autoload_*.php` files, these were brought up in a [post from Ralph](http://ralphschindler.com/2011/09/19/autoloading-revisited), based on discussions he and I have had. The idea behind these is that we can satisfy several typical use cases for modules:
...
--------------------
 ...
 The MVC prototype has grown tremendously, in large part due to the efforts of [Evan Coury](http://evan.pro/) and his work on the Module Manager. The module manager now does the following:
...
- The ability to replace the default routing and dispatch listeners with custom listeners.
 ...
 This last point allowed me to prototype a [ZF1-emulation layer in the new ZF2 paradigm](https://github.com/weierophinney/zf-quickstart/tree/features/zf2_mvc-zf1_emulation). This effort was surprisingly easy, and helped illustrate how flexible the prototype can be.

 Additional work on the MVC centered around error handling, and providing a simple mechanism for discovering and handling errors. The [zf-quickstart example](https://github.com/weierophinney/zf-quickstart/tree/features/zf2-mvc-module) showed that this largely eliminates the need for an `ErrorController`.
...
---------------------
 ...
 This work should hit master this week, and will be compatible with the [docblock extension](http://pecl.php.net/package/docblock).
...
----------
 ...
 Also, we have an IRC meeting _this week_, 28 September 2011 at 17:00 UTC. The [agenda](http://framework.zend.com/wiki/display/ZFDEV2/2011-09-28+Meeting+Agenda) is online, but could potentially use some more topics and votes. Talk to you Wednesday!